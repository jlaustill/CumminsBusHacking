...
public:
    ...
    volatile void push (const T i) volatile;
...
private:
    ...
    void exit (const char * m) volatile;

    // led blinking method in case of error.
    void blink () volatile const;
...
// push an item to the queue.
template<typename T>
volatile void QueueList<T>::push (const T i) volatile {
...
// check if the queue is empty.
template<typename T>
bool QueueList<T>::isEmpty () volatile const {
    return head == NULL;
}
...
// exit report method in case of error.
template<typename T>
void QueueList<T>::exit (const char * m) volatile {
...
// led blinking method in case of error.
template<typename T>
void QueueList<T>::blink () volatile const {
